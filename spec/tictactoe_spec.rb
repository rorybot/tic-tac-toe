...
describe TicTacToe do
  ...
  describe 'taking turns' do
    let(:player1) { double('Player1', turns: 1, place_piece: 'x', symbol: 'X', already_moved: false, set_movement: true) }
    let(:player2) { double('Player2', turns: 0, place_piece: 'x', symbol: 'O', already_moved: false, set_movement: true) }
    ...
    it 'wont let you player 1 play two in a row' do
      allow(player1).to receive(:already_moved).and_return(true)
      expect(tictactoe.take_turn(player1, 'x', 'x')).to eq 'Not ye turn laddy'
    end
  end

  describe 'trying to overwrite' do
    let(:player1) { double('Player1', turns: 1, place_piece: 'x', symbol: 'X', already_moved: false) }
